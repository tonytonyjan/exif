...
void each_entry(ExifEntry *ee, void *self_ptr){
  exif_entry_dump(ee, 2);
  VALUE *self, value;
  const char *ivar_name;

  self = (VALUE*)self_ptr;
  ivar_name = exif_entry_to_ivar(ee);
  value = exif_entry_to_value(ee);
...
VALUE exif_entry_to_value(ExifEntry *entry){
  ...
  case EXIF_FORMAT_SHORT:
    if(entry->components > 1){
      ...
      for(i = 0; i < entry->components; i++)
        rb_ary_push(ret, INT2FIX(exif_get_short(entry->data + exif_format_get_size(entry->format) * i, order)));
  ...
  case EXIF_FORMAT_SSHORT:
    if(entry->components > 1){
      ret = rb_ary_new2(entry->components);
      for(i = 0; i < entry->components; i++)
        rb_ary_push(ret, INT2FIX(exif_get_sshort(entry->data + exif_format_get_size(entry->format) * i, order)));
